...
- **设备组与多套服务器配置**
  - 服务器配置只维护可复用的 ID Server、Relay Server 和 Server Key。
  - 每个设备组将服务器配置、无人值守、Root 执行器和加密固定密码作为一套完整策略。
  - 设备只选择所属组；只有未分组设备使用唯一的默认设备组，已分组但策略不可用时返回无管理策略并明确告警。
  - 提供最终生效策略及来源预览；固定密码加密保存，管理查询和日志不返回明文或密文。

- **设备组无人值守管理**
  - Web 按设备组配置无人值守及 Root 执行器；`auto` 依次探测 `su`、`testsu`，也可明确指定执行器。
  - 关闭无人值守时保留组内密码密文，但向客户端下发空的管理端固定密码。
