...
- **Web 设备名称与地址簿同步**
  - 在设备管理中编辑、清空和搜索设备名称，支持中文。
  - 管理员明确选择发布到哪些账号的个人地址簿；同步时保留已有条目的密码、标签等信息。
  - 受管名称以 Web 为准，客户端回写旧别名不会覆盖管理端设置。
  - 设备名称不改变连接 ID；连接方使用官方客户端登录并刷新地址簿后，可按名称查找设备。

- **受管连接 ID**
  - 在 Web 设备管理中为已注册的受管 Android 设备下发一次性连接 ID，并显示等待应用、已生效或失败状态。
  - 客户端复用 RustDesk 现有改 ID协议向当前 ID Server 申请，只有服务端确认成功后才保存新 ID；失败保留旧 ID并允许重试。
  - 保留 hbbs 1.1.11，不修改 hbbs；连接方官方客户端配置同一套 ID Server、Key 和 Relay 后，可直接输入已生效 ID连接，无需重新打包。
  - 设备身份使用数字 ID、公钥和 UUID 稳定绑定，连接 ID 变化不会创建重复设备记录；成功后同步更新受管地址簿。
...
- **构建与本地验证**
  ...
  - 补充设备生命周期、名称同步、连接 ID、权限状态、事务回滚及并发一致性测试。
...
- 设备名称只用于 Web 和地址簿识别；只有已生效的连接 ID 可用于直接连接。受控端需要定制 Android APK，连接方官方客户端无需定制。
- 连接 ID 流程使用 hbbs 1.1.11 已有协议，但真实下发、ID Server 更新和官方客户端直连仍需部署后联调验收。
