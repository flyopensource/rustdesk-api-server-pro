...
## 设备组完整策略调整（2026-09-08）

- 服务器配置收敛为 ID Server、Relay Server 和 Server Key；固定密码移入设备组并继续加密保存。
- 设备组统一管理服务器配置、无人值守、固定密码和 Root 执行器，设备只保留所属组。
- 已分组设备只解析所属组；只有 `group_id = 0` 的未分组设备使用默认组。所属组或服务器配置不可用时返回无管理策略并告警。
- 默认组开关已并入设备组编辑表单；唯一默认关系与组策略在同一事务中保存。
- 删除单设备服务器配置、单设备无人值守和全局服务器配置接口；Web 只允许切换设备组，并只读展示生效值和客户端实际状态。
- 关闭无人值守时策略包不携带固定密码；组内密文保留供再次开启使用。管理查询和调试请求日志不输出密码。
- Root 执行器默认 `auto`，客户端实际依次探测 `su`、`testsu`；管理端允许按设备组明确设置执行器。
- 旧策略数据不迁移、不兼容；部署时使用全新策略数据重新建立服务器配置、设备组、密码和默认组。
- 默认开关后续收敛到设备组表单，与组策略同事务保存；只有未分组设备使用默认组，已分组但策略不可用时返回无管理策略。
- 验证：Go 全量测试、race、vet、Web TypeScript 类型检查、ESLint、生产构建和模拟 API 页面测试通过。2026-09-08 已发布真实 API 和 Web，安装同一签名 APK 后两块主板分别验证 `su`/`testsu` 无人值守成功；默认开关后续版本已验证线上 API 和前端构建一致。
