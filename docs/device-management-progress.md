...
范围：开发阶段按当前数据结构实现，不做历史版本迁移。每步本地自测并单独提交；真实 API、hbbs 1.1.11、官方客户端和主板的端到端联调暂缓。
...
- ARMv7 Rust/Flutter Release APK 已完成构建及签名、ABI、清单权限和 SHA256 验证。未完成真机授权、远程文件传输和 Android 11+ 设备验证；已连接主板为 Android 7.1/API 25，本步未安装或替换 APK、未执行授权命令。

## 第 3 步：设备名称与地址簿发布

- Web 设备列表提供设备名称列、搜索、编辑弹窗和发布目标多选；底层沿用 `device.alias`，支持中文、修剪首尾空格、清空，拒绝控制字符和超过 128 字的名称。
...
- 官方客户端的逐条和整本地址簿接口仍可读取名称；回写不能覆盖 Web 名称或删除受管条目，但逐条更新仍可保存密码/标签。设备名称与连接 ID 是两套独立语义。
...
- 设备管理以数字 ID、公钥和 UUID 作为稳定身份；客户端实际 RustDesk ID 变化时更新同一设备记录，不重复注册。
- 管理接口只接受已启用受管设备和 6–16 位规范 ID；检查设备表的当前/待分配 ID 及受管地址簿冲突。已生效后不可再次修改，失败后可重试。
- 签名策略升级为版本 2，以 `device_id + UUID` 绑定，并携带连接 ID 请求状态和独立 revision；签名心跳上报 applied/failed，服务端拒绝旧 revision 结果。
- Android 客户端复用 RustDesk 已有的 Custom ID 变更流程，把 `old_id + new_id + UUID` 发往当前 ID Server。只有全部服务器确认成功才保存新 ID 并重启 rendezvous mediator；失败保留旧 ID。
- 身份凭据缓存改为 API Server + UUID，不因 ID 修改重复注册。服务端确认 applied 后在事务内更新实际 ID、受管地址簿条目和审计。
- Web 将原“RustDesk ID”展示为“连接 ID”，显示当前值、目标值、pending/applied/failed 状态和失败原因；已生效后隐藏修改入口。原“设备别名”改称“设备名称”。
- 本地验证：Go 全量测试、目标 race 测试、ARMv7 release 库跨编译、Web 类型检查、生产构建和 Chrome Mock 页面测试通过。
- 未执行：真实 API/hbbs 1.1.11/Android 策略下发与结果回报、官方客户端直连、占用与断网等真实失败场景。
...
- Web TypeScript 检查、生产构建通过；Chrome 隔离上下文及全模拟接口页面测试通过，覆盖连接 ID 校验/转小写/pending、取消编辑不提交、保存设备名称与目标参数、停用和删除。
- Dart 分析只有 3 条既有 MaterialStateProperty 弃用提示；受管 Android Kotlin 编译和默认/受管清单合并通过。
- ARMv7 Android Rust `cargo ndk check --release --lib --features flutter,hwcodec` 通过（有既有编译警告），并完成完整 ARMv7 Release APK 构建。APK 已验证 v1/v2 签名、仅含 `armeabi-v7a` 原生库、声明受管文件权限且 SHA256 一致。
- Linux amd64 静态服务端与 Web ZIP 已重新构建，静态 ELF、隔离 SQLite 建表、压缩包完整性和 SHA256 检查通过；未部署。
...
| 4 单设备删除 | API | `0290142` |
| 5 稳定设备身份 | API | `2f94bad` |
| 5 连接 ID 分配与结果协调 | API | `413e6f6` |
| 5 Android 应用连接 ID | 客户端 | `6bb1861be` |
| 5 Web 连接 ID 管理 | API | `d06ccd9` |

第 6 步由包含本节的测试/文档提交记录。
